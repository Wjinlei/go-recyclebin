...
func (bin recycleBinStruct) Remove(filePath string) (fileNameInRecycleBin string, err error) {
	...
	// 加载info文件
	iniClass := NewIni()
	config, err := iniClass.Create(infoPathInRecycleBin)
	if err != nil {
		return "", err
	}
	config.File.Section("Trash Info").Key("Path").SetValue(filePathAbs)
	config.File.Section("Trash Info").Key("Name").SetValue(filepath.Base(filePathAbs))
	config.File.Section("Trash Info").Key("DeletionDate").SetValue(golib.GetNowTime())
...
func (bin recycleBinStruct) Restore(fileNameInRecycleBin string) error {
	...
	// 加载Info文件
	iniClass := NewIni()
	config, err := iniClass.LoadFile(infoPathInRecycleBin)
	if err != nil {
		return err
	}

	// 读取文件真实路径
	realPathObject, err := config.File.Section("Trash Info").GetKey("Path")
	if err != nil {
		return err
	}
